...
Outside Docker, API and web still run as two separate processes on ports 4000
and 3000 (see Quick start below) — Docker is what packages them into one
container, one port. See [DOCKER_DEPLOYMENT.md](DOCKER_DEPLOYMENT.md) for the
full local-Docker and production walkthrough.
...
point at a real (or your own throwaway) Postgres instance, and you still run
the migration once yourself (`npm run migrate --workspace=apps/api`, from
outside Docker, against the same `DATABASE_URL`).

See [DOCKER_DEPLOYMENT.md](DOCKER_DEPLOYMENT.md) for the full local-Docker
walkthrough and production deployment steps.
